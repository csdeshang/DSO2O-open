...
if($action == 'db') {
    if($ispost) {
        ...
        if(empty($error)) {
            ...
            if($_POST['initdata']){
                mysqli_query($link,"update {$db['prefix']}goods set store_name = '{$store_name}' where store_id = 1");
                mysqli_query($link,"update {$db['prefix']}goodscommon set store_name = '{$store_name}' where store_id = 1");
            }
            //配置数据库
