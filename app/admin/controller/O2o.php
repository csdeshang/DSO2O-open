...
class O2o extends AdminControl {
    ...
    public function other() {
        ...
        if (!request()->isPost()) {
            ...
            if ($result) {
                ...
                $this->success(lang('ds_common_save_succ'), 'O2o/other');
            } else {
    ...
    public function distance_price() {
        ...
        if (!request()->isPost()) {
            $list_config = rkcache('config', true);
            $list_config['o2o_distance_price'] = unserialize($list_config['o2o_distance_price']);
            View::assign('list_config', $list_config['o2o_distance_price']);
            $this->setAdminCurItem('distance_price');
            return View::fetch();
        } else {
            $update_array = array();
            $update_array['o2o_distance_price'] = $this->formatO2oDistancePrice();
            $result = $config_model->editConfig($update_array);
            if ($result) {
                dkcache('config');
                $this->log(lang('ds_edit') . lang('o2o_distance_price'), 1);
                $this->success(lang('ds_common_save_succ'), 'O2o/distance_price');
            } else {
                $this->log(lang('ds_edit') . lang('o2o_distance_price'), 0);
            }
        }
    }
    ...
    public function weight_price() {
        ...
        if (!request()->isPost()) {
            $list_config = rkcache('config', true);
            $list_config['o2o_weight_price'] = unserialize($list_config['o2o_weight_price']);
            View::assign('list_config', $list_config['o2o_weight_price']);
            $this->setAdminCurItem('weight_price');
            return View::fetch();
        } else {
            $update_array = array();
            $update_array['o2o_weight_price'] = $this->formatO2oWeightPrice();
            $result = $config_model->editConfig($update_array);
            if ($result) {
                dkcache('config');
                $this->log(lang('ds_edit') . lang('o2o_weight_price'), 1);
                $this->success(lang('ds_common_save_succ'), 'O2o/weight_price');
            } else {
                $this->log(lang('ds_edit') . lang('o2o_weight_price'), 0);
            }
        }
    }
    ...
    public function time_price() {
        ...
        if (!request()->isPost()) {
            $list_config = rkcache('config', true);
            $list_config['o2o_time_price'] = unserialize($list_config['o2o_time_price']);
            View::assign('list_config', $list_config['o2o_time_price']);
            $this->setAdminCurItem('time_price');
            return View::fetch();
        } else {
            $update_array = array();
            $update_array['o2o_time_price'] = $this->formatO2oTimePrice();
            $result = $config_model->editConfig($update_array);
            if ($result) {
                dkcache('config');
                $this->log(lang('ds_edit') . lang('o2o_time_price'), 1);
                $this->success(lang('ds_common_save_succ'), 'O2o/time_price');
            } else {
                $this->log(lang('ds_edit') . lang('o2o_time_price'), 0);
            }
        }
    }
...
//                }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                'name' => 'distance_price',
                'text' => lang('o2o_distance_price'),
                'url' => url('O2o/distance_price')
            ),
            array(
                'name' => 'weight_price',
                'text' => lang('o2o_weight_price'),
                'url' => url('O2o/weight_price')
            ),
            array(
                'name' => 'time_price',
                'text' => lang('o2o_time_price'),
                'url' => url('O2o/time_price')
            ),
            array(
                'name' => 'other',
                'text' => lang('o2o_other'),
